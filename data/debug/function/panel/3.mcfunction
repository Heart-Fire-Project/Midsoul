...
tellraw @s ["Version 2.0.0-Beta / Plus 7 (",{score:{name:"$build",objective:"data"}},")"]
...
tellraw @s [{text:"",color:"#7367F0"},{text:"» ",bold:true},{translate:"ms.debug.panel",fallback:"午夜灵魂调试面板 V2"},{text:" » ",bold:true},{translate:"ms.debug.init",fallback:"[全局重置]",color:"red",click_event:{action:"run_command",command:"trigger detect.interact set -999"}}," ",{translate:"ms.debug.leave_admin",fallback:"[退出 admin]",color:"light_purple",click_event:{action:"run_command",command:"trigger detect.interact set -777"}}]
