# 倒计时通项
$execute if score $prepare data matches $(start).. store result bossbar midsoul:info value run scoreboard players get $0_start tick.global
...
$execute if score $0_start tick.global matches ..10 if score $prepare data matches $(start)..10 run bossbar set midsoul:info name [{"translate":"ms.bossbar.0.start","fallback":"人数已达标 | 游戏将在 %s 秒后开始","color":"aqua","with":[{"score":{"objective":"tick.global","name":"$0_start"},"color":"#50A7D9"}]}]

# 人数超限倒计时
execute if score $prepare data matches 11.. run bossbar set midsoul:info color green
execute if score $0_start tick.global matches ..10 if score $prepare data matches 11.. run bossbar set midsoul:info name [{"translate":"ms.bossbar.0.overload","fallback":"人数已超限 | 游戏将在 %s 秒后开始","color":"#33FF33","with":[{"score":{"objective":"tick.global","name":"$0_start"},"color":"#20C020"}]}]
