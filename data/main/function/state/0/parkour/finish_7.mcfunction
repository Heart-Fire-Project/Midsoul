...
tellraw @s[scores={stat.parkour_7=..0}] ["   ",{translate:"ms.parkour.pb",fallback:"个人最佳"}," » ","--:--.--"]
tellraw @s[scores={stat.parkour_7=1..}] ["   ",{translate:"ms.parkour.pb",fallback:"个人最佳"}," » ",{storage:"ms:string",nbt:"result",interpret:true}]

# 个人最佳
execute if score @s temp2 < @s stat.parkour_7 run scoreboard players set @s tick.parkour -54
scoreboard players set @s[scores={stat.parkour_7=..0}] tick.parkour -54
scoreboard players operation @s[scores={tick.parkour=-54}] stat.parkour_7 = @s temp2
