...
# 升级进程
scoreboard players remove @a[scores={tick.global=1..}] tick.global 1
scoreboard players add @a[scores={tick.global=..-1}] tick.global 2
execute as @a[scores={tick.global=..-1}] run function main:state/0/exp/loop
execute as @a[scores={tick.global=0,exp.temp=1..}] run function main:state/0/exp/loop
