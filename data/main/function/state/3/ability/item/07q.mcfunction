# 第一次丢弃
execute if score @s tick.item matches 0.. run title @s[scores={setting.ability_status=2}] actionbar {translate:"ms.item.drop",fallback:"再次按 [%s] 以丢弃",with:[{keybind:"key.drop"}],color:"#33FFAA"}
...
execute if score @s tick.item matches -3900..-1 run scoreboard players set @s tick.item 0