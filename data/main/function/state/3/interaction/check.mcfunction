...
execute as @s[tag=blue_hint,tag=show_title] run title @s actionbar [{"translate":"ms.hint.blue","fallback":"长按 [%s] 以收集","with":[{"keybind":"key.sneak"}],"color":"blue"}]
...
# 刷新进程
scoreboard players reset @s[tag=!interacting] tick.global
...
scoreboard players operation @s[tag=interacting] tick.global += @s temp

# 收集完成
execute as @s[tag=blue_interact,scores={tick.global=700..}] as @e[tag=blue,distance=..0.7] at @s run function main:state/3/interaction/blue with storage ms:map
execute as @s[tag=gold_interact,scores={tick.global=1400..}] as @e[tag=gold,distance=..0.7] at @s run function main:state/3/interaction/gold
execute as @s[tag=gray_interact,scores={tick.global=1000..}] as @e[tag=gray,distance=..0.7] at @s run function main:state/3/interaction/gray
execute as @s[tag=E02,scores={tick.global=700..}] as @e[tag=fake_blue,distance=..0.7] at @s run function main:state/3/echo/02a