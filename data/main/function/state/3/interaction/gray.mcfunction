tag @a[distance=..0.7,tag=gray_interact,scores={tick.global=1000..}] add interact_fin
...
# 灵魂效果
function base:random {max:"7",min:"1"}
execute if score $echo data matches 4 run function main:state/3/echo/04
execute if entity @a[team=soul,distance=..0.7,tag=gray_interact,scores={tick.global=1000..,item=1..}] run effect give @p[team=guardian] glowing 7 0
execute as @r[team=soul,distance=..0.7,tag=gray_interact,scores={tick.global=1000..,item=0}] run scoreboard players operation @s item = $random temp2
...
# 判定：开启灵魂宝物盒时
execute if score $echo data matches 4 as @a[team=guardian,tag=interact_fin,scores={tick.skill=1..}] run scoreboard players remove @s tick.skill 4000
