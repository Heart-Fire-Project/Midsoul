...
# 刷新传送门开启进度
scoreboard players add $4_process_color tick.global 1
...
# 计时部分
scoreboard players remove $4_portal tick.global 1
scoreboard players add $stat_gametime tick.global 1
scoreboard players add $4_process tick.global 1
...
execute as @a if block ~ ~ ~ air if data entity @s {OnGround:0b} run scoreboard players add @s tick.off_ground 1
