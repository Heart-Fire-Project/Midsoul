...
execute if score $num temp matches 1.. run scoreboard players add $4_chargesound tick.global 1
execute if score $num temp matches 1.. if score $4_chargesound tick.global matches 9.. run playsound block.sculk.charge player @a 0 1000000 0 1000000
execute if score $4_chargesound tick.global matches 11.. run scoreboard players set $4_chargesound tick.global 0

# 逸散进度结算
scoreboard players operation $minus temp = $interact_speed setting
scoreboard players operation $minus temp /= #5 data
execute unless score $num temp matches 1.. if score @s tick.global matches 1.. run scoreboard players operation @s tick.global -= $minus temp
execute if score @s tick.global matches ..-1 run scoreboard players set @s tick.global 0
...
execute as @a[tag=purple_interact,tag=interacting,distance=..0.7] run scoreboard players operation $plus temp += @s tick.global
...
execute if score $num temp matches 1.. run scoreboard players operation @s tick.global += $plus temp

# 充能完毕？
execute if score @s tick.global matches 8000.. run function main:state/4/charge/finish