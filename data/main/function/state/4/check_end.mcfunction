...
execute unless entity @a[team=soul,scores={state=0}] unless entity @e[tag=open_purple] run scoreboard players set $guardian_offense data 120
...
# 时间警告
execute if score $4_portal tick.general matches ..600 if score $4_finale state matches 0 run title @a title ""
execute if score $4_portal tick.general matches ..600 if score $4_finale state matches 0 run title @a subtitle [{text:"» ",color:"#C13435",bold:true},{translate:"ms.title.4.warn",fallback:"剩余 30 秒",bold:false},{text:" «",bold:true}]
execute if score $4_portal tick.general matches ..600 if score $4_finale state matches 0 run playsound event.mob_effect.raid_omen player @a 0 1000000 0 120000
