...
# 选定
execute as @a[tag=game_player] run scoreboard players operation @s temp2 = @s temp
execute as @a[tag=game_player] run scoreboard players operation @s temp2 -= $random temp2
scoreboard players reset @a[tag=game_player,scores={temp2=..0}] temp2
scoreboard players set $min temp2 1010000
scoreboard players operation $min temp2 < @a[scores={temp2=1..},team=prepare] temp2
