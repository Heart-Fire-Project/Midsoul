...
execute if score $player temp matches ..5 run scoreboard players set $target temp 1
execute if score $player temp matches 6..8 run scoreboard players set $target temp 2
execute if score $player temp matches 9.. run scoreboard players set $target temp 3

# 计算踢出守卫者人数
scoreboard players set $guardian temp 0
execute as @a[scores={state=2}] run scoreboard players add $guardian temp 1
execute store result storage ms:temp num int 1 run scoreboard players operation $guardian temp -= $target temp
...
scoreboard players operation $player temp -= $target temp
