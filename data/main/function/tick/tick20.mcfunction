...
execute if score $admin_tip tick.general matches ..0 run tellraw @a[team=admin] [{text:"» ",bold:true,color:"light_purple"},{translate:"ms.info.leave_admin",fallback:"点这里以退出 admin",bold:false,"click_event":{action:"run_command",command:"trigger detect.interact set -999"}}]
