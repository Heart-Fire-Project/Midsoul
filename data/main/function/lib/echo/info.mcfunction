...
execute if score $echo data matches 5 run tellraw @s [{translate:"ms.echo.5.desc",fallback:"所有灵魂的移速提升至 110%%，交互速度 -20%%，自身大小 -30%%"}]
execute if score $echo data matches 6 run tellraw @s [{translate:"ms.echo.6.desc",fallback:"所有玩家的移速提升至 150%%，每进入下一阶段时该倍率提升 25%%"}]
