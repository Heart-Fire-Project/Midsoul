...
execute if score $echo data matches 1 run tellraw @s [{translate:"ms.echo.1.desc",fallback:"每过 45~80 秒，清除已有标记并随机标记 2~4 个灵魂；被标记的灵魂将持续发光，未被标记的灵魂不会受到伤害但其交互速度 -50%%"},"\n",{translate:"ms.echo.1.desc.sub",fallback:"对局开始时立刻触发一次，剩余灵魂 <3 人时回响失效",color:"gray"}]
