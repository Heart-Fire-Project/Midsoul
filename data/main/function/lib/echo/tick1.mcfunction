# 持续性效果
execute if score $echo data matches 1 if score $echo tick.general matches 0.. if data storage ms:mode {logic:"1"} run scoreboard players remove @a[team=soul,scores={state=1,tick.general=1..}] tick.general 25
