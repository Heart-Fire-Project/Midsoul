...
# 随机标记 2~4 个灵魂 | 仅 >5 灵魂存活时为 3 标记，>7 灵魂存活时为 4 标记
execute if score $alive data matches 7..8 run tag @a[team=soul,limit=4,sort=random] add echo_target
execute if score $alive data matches 5..6 run tag @a[team=soul,limit=3,sort=random] add echo_target
