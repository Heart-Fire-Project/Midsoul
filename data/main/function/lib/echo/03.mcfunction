...
execute if score $E03 temp matches 1 run scoreboard players operation $num temp = $relic_max data
execute if score $E03 temp matches 1 store result storage ms:temp max int 1 run scoreboard players remove $num temp 1
execute if score $E03 temp matches 1 run data merge storage ms:temp {min:1}
