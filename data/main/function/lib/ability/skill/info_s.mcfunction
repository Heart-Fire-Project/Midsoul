...
tellraw @s[scores={skill=6}] [{translate:"ms.skill",fallback:"[技能] ",color:"#5599FF"},{translate:"ms.skill.006",fallback:"济困扶危"}]
tellraw @s[scores={skill=7}] [{translate:"ms.skill",fallback:"[技能] ",color:"#5599FF"},{translate:"ms.skill.007",fallback:"同气连枝"}]
...
tellraw @s[scores={skill=6}] [{text:"⚡ 70  ⌚ 7 > ",color:"#5599FF"},{translate:"ms.tag.survive",fallback:"生存"}]
...
tellraw @s[scores={skill=6}] [{translate:"ms.skill.006.desc",fallback:"使周围 2 格内血量最低的灵魂获得 4 伤害吸收，并使其在 20 秒内回复 8 血量"},"\n",{translate:"ms.skill.006.desc.sub",fallback:"每次使用该技能都会使技能冷却 +10 秒，至多提升至 120 秒",color:"gray"}]
