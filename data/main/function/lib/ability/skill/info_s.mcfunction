...
tellraw @s[scores={skill=4}] [{translate:"ms.skill.004.desc",fallback:"若距离守卫者不足 20 格，则自身移速 +140%% 且使 8 格内的守卫者无法攻击，或自身移速 -60%%；若距离守卫者大于 20 格，则使最近的守卫者发光且移速 -60%%，或自身传送至最近的守卫者附近并移速 -90%%"},"\n",{translate:"ms.skill.004.desc.sub",fallback:"触发上述负面效果的概率初始为 5%%，但每次使用该技能都会 +7%%，至多提升至 40%%",color:"gray"}]
