...
tellraw @s[scores={skill=3}] [{translate:"ms.skill.103.desc",fallback:"移速 +20%%；标记最远的灵魂传送至其附近并令其发光 3 秒，技能持续时间内被标记的灵魂会持续留下尾迹"}]
tellraw @s[scores={skill=4}] [{translate:"ms.skill.104.desc",fallback:"为一个随机的灵魂宝物盒附加%s效果，若没有未被诅咒的灵魂宝物盒则生成 2 个；移速 +40%%，场上每有一个被诅咒的灵魂宝物盒额外 +20%%，上限 +140%%",with:[{translate:"ms.tooltip.curse",fallback:"诅咒",underlined:true,"hover_event":{action:"show_text",value:[{translate:"ms.tooltip.curse",fallback:"诅咒",color:"red"},"\n",{translate:"ms.tootip.curse.detail",fallback:"该效果可被施加给灵魂宝物盒。\n\n有灵魂经过时使其发光且移速 -15%%，开启后使其发光 7 秒，并在 3 秒内移速 -30%%；有守卫者经过时使全体灵魂发光且自身移速 +20%%，开启后使全体灵魂发光 7 秒并在 15 秒内速度 +40%%",color:"white"}]}}]}]
tellraw @s[scores={skill=5}] [{translate:"ms.skill.105.desc",fallback:"移速 +40%%；无法直接攻击，但可以释放剑气造成范围伤害；技能结束后进入擦刀状态"},"\n",{translate:"ms.skill.105.desc.sub",fallback:"选中武器并短按 [%s] 以释放剑气，对周围 3 格内的所有灵魂造成可加成的 5 伤害\n剑气若造成伤害则技能时长延长 6 秒，否则技能立刻结束；至多延长 3 次",color:"gray",with:[{keybind:"key.drop"}]}]