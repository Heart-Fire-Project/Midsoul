...
execute as @a[team=soul] if score @s entity_id = $entity_id temp run tag @s add S102t
execute as @a[tag=S102t] run function base:get_pos
...
# 获取粒子运动时间 | 速度为 20 格/秒
function base:caculate/selector_distance {target1:"@s",target2:"@p[tag=S102t]"}
execute store result storage ms:temp duartion int 1 run scoreboard players get $front temp2

# 执行粒子生成
function main:lib/ability/skill/102a with storage ms:temp
