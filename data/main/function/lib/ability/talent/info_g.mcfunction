...
$tellraw @s[scores={talent_$(num)=4}] [{text:"⚡ 75 > ",color:"gold"},{translate:"ms.tag.kill",fallback:"杀戮"}]
...
$tellraw @s[scores={talent_$(num)=3}] [{translate:"ms.talent.103.desc",fallback:"擦刀状态持续时间 -15%%"}]
$tellraw @s[scores={talent_$(num)=4}] [{translate:"ms.talent.104.desc",fallback:"天赋触发时使全体灵魂在 5 秒内陷入黑暗；生效期间自身隐身"},"\n",{translate:"ms.talent.104.desc.sub",fallback:"距离灵魂 8 格以内时结束；攻击到灵魂时重置",color:"gray"}]
...
$tellraw @s[scores={talent_$(num)=9}] [{translate:"ms.talent.109.desc",fallback:"位于未开启的传送门周围 3 格内时，其交互进度流失速度提升至 250%"}]