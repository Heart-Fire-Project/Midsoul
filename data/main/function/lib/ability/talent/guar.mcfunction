...
$tellraw @s[scores={talent_$(num)=4}] [{translate:"ms.talent.104.desc",fallback:"天赋触发时使全体灵魂在 5 秒内陷入黑暗；生效期间自身隐身，并使距离 40 格外的灵魂发光"},"\n",{translate:"ms.talent.104.desc.sub",fallback:"距离 20 格内有灵魂时结束；攻击到灵魂时重置",color:"gray"}]
$tellraw @s[scores={talent_$(num)=5}] [{translate:"ms.talent.105.desc",fallback:"使距离你较远的至多 2 个灵魂发光"}]
$tellraw @s[scores={talent_$(num)=6}] [{translate:"ms.talent.106.desc",fallback:"擦刀状态结束时，使最近的灵魂发光"}]
$tellraw @s[scores={talent_$(num)=7}] [{translate:"ms.talent.107.desc",fallback:"污染场上的一个碎片；被污染的碎片将在被收集时发射一个烟花"},"\n",{translate:"ms.talent.107.desc.sub",fallback:"至多同时对 3 个碎片生效",color:"gray"}]