...
# 若可生成，再进行
$execute if score $chest_current temp < $chest_max temp run function main:lib/event/summon/gray {num:"$(chest_summon)"}

# 成功生成后播报
execute if score $chest_current temp < $chest_max temp run tellraw @a [{text:"» ",color:"#CCCCCC",bold:true},{translate:"ms.info.chest_refresh",fallback:"已生成数个灵魂宝物盒",bold:false}]
