...
scoreboard players set $score temp 600
...
tellraw @s [{text:" +",color:"#009295"},{score:{name:"$score",objective:"temp"}}," | ",{translate:"ms.rating.collect",fallback:"碎片凝淬"}]
scoreboard players operation $goal temp /= #4 data
scoreboard players set $score temp 30
scoreboard players operation $score temp *= $value temp
scoreboard players operation $score temp /= #4 data
execute if score @s temp.rated_collect = $goal temp run tellraw @s [{text:" +",color:"#009295"},{score:{name:"$score",objective:"temp"}}," | ",{translate:"ms.rating.collect",fallback:"碎片凝淬"}," 25%"]
execute store result score $goal temp run data get storage ms:map shard_goal
...
scoreboard players operation $score temp *= $value temp
scoreboard players operation $score temp /= #4 data
execute if score @s temp.rated_collect = $goal temp run tellraw @s [{text:" +",color:"#009295"},{score:{name:"$score",objective:"temp"}}," | ",{translate:"ms.rating.collect",fallback:"碎片凝淬"}," 40%"]