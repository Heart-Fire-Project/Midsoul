execute if score $state data matches 3 run tellraw @s [{text:" +300 | ",color:"#D13D18"},{translate:"ms.rating.offense.s",fallback:"门扉未起"}]
execute if score $state data matches 4 unless entity @e[tag=open_purple] run tellraw @s [{text:" +240 | ",color:"#D13D18"},{translate:"ms.rating.offense.a",fallback:"遥遥无期"}]
...
execute if entity @e[tag=open_purple] run scoreboard players add $sec temp2 100
execute if entity @e[tag=open_purple] if score $sec temp2 matches 241.. run scoreboard players set $sec temp2 240
