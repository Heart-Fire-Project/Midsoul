execute if score $state data matches 3 run tellraw @s [{text:" +240 | ",color:"#D13D18"},{translate:"ms.rating.offense.s",fallback:"门扉未起"}]
...
execute if entity @e[tag=open_purple] if score $sec temp2 matches 201.. run scoreboard players set $sec temp2 200
