...
tellraw @a[team=alive,scores={skill=2}] [" ",{"translate":"ms.skill.002.desc","fallback":"持续发光且速度大幅降低, 结束后使一个灵魂解除垂死状态, 若无陷入垂死状态的灵魂, 则灵魂守护者持续发光且速度小幅降低"}]
...
tellraw @a[team=protect,scores={skill=2}] [" ",{"translate":"ms.skill.102.desc","fallback":"技能期间每有一个碎片被收集, 你都会在技能后获取一小段高额伤害与移速加成"}]
